...
async function validatePayment(
  ...
  try {
    ...
    let addressTopay = await walletNetworkUser.findOne({
      userId: userId,
    ...
    switch (network.networkId) {
      case "tron":
        ...
        for (
          let iData = 0;
          iData < transactionLog.transfersAllList.length;
          iData++
        ) {
          ...
          if (x.to_address.toLowerCase() == addressTopay.address.toLowerCase())
            if (
              x.contract_address.toLowerCase() == asset.address.toLowerCase()
            ) {
              //network.deposit_address.toLowerCase() validamos que el que recibio el token es nuestra wallet de tx.
              //primero deberiamos validar que sea el token del asset
      ...
      case "ethereum":
        ...
        if (
          ...
          if (
            transactionLog.tokenContract.toLowerCase() ==
            asset.address.toLowerCase()
          ) {
            // validamos que el que recibio el token es nuestra wallet de tx.
            //primero deberiamos validar que sea el token del asset
            //validar la cantidad de token
            let quantity = divideByDecimals(
      ...
      case "bitcoin":
        ...
        for (let i = 0; i < transactionLog.outputs.length; i++) {
          //recorrer los outputs (salidas de btc a wallets)
