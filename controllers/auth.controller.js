...
module.exports = {
  async login(req, res) {
    try {
      ...
      // Template Logger for logging time login

      // To track the frequency and time of logins, we need to log each login event with a timestamp. We can then analyze this data to calculate login frequencies and determine the most common login times.
      ...
      //     loginTimes.push(login.timestamp);
      //   });
      // });

      // logger.info(`Log-in Count: ${loginCount}`)
      // logger.info(`Log-in Times: ${loginTimes}`)
  ...
  async register(req, res) {
    try {
      ...
      // Template Logger for logging number of sign ups
      // To calculate the number of sign-ups, count the total number of user records in the database.

      // const totalSignup = await UserModel.countDocuments();

      // logger.info(`New sign-up: ${user.user_name}`);
      // logger.info(`Sign-up count: ${totalSignup}`);
