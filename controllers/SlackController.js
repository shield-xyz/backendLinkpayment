...
//         channel: channelId, // Replace with your actual channel ID
//         text: `Error encountered: ${error.message}\nStack trace: ${error.stack}`
//       });
//     } catch (slackError) {
...
async function listBalances() {
  ...
  for (let i = 0; i < users.length; i++) {
    ...
    let balances = await balanceModel
      .find({ userId: userId._id, amount: { $gt: 0 } })
...
async function generateWithDraw(amount, balanceId) {
  ...
  if (amount > balance.amount) {
    await sendMessage(
      "the amount exceeds the balance, balance: " + balance.amount
    );
    return;
  }
  ...
  let userConf =
    await ConfigurationUserController.userConfigForUserAndConfigName(
      balance.userId,
      CONFIGURATIONS.EMAIL_NAME
    );
    ...
    for (const channel of result.channels) {
      ...
      try {
        ...
        if (error.data.error === "not_in_channel") {
          console.log(
            `El bot no es miembro del canal: ${channel.name}, uniéndose...`
          );
          ...
          console.error(
            `Error al obtener historial del canal ${channel.name}:`,
            error
          );
        }
      }
...
async function listWithDraws() {
  ...
  block.push({
    type: "header",
    text: { type: "plain_text", text: "WithDraws List" },
  }); //header
  let section = {
    type: "section",
  };
  ...
  wts.map((withdraw) => {
    ...
    fields.push({
      ...
      text:
        "*" +
        padRightTo(withdraw.amount + " " + withdraw.asset.symbol, 15) +
        "* | " +
        withdraw.status,
    });
  });
