...
const TransactionLogController = {
  async createTransaction(data) {
    try {
      ...
      // logger.info(`Transaction processed: amount=${data.amount_str}, currency=${data.symbol}, date=${new Date(transactionLog.timestamp)}`);
  ...
  async updateTransaction(id, data) {
    const transactionLog = await TransactionLog.findOneAndUpdate({ id }, data, {
      new: true,
      upsert: true,
    });
    return transactionLog;
