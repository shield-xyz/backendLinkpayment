...
const {
  ...
  formatCurrency,
  removeCeros,
} = require("../utils"); // Asumiendo que tienes un manejador de errores
...
const {
  getTokenTransactionsEth,
  getTokenTransactionsPolygon,
  getTokenTransactionsSolana,
} = require("../utils/EthereumNetworkUtils");
...
async function getTransactionWalletTron(
  walletAddress = process.env.WALLET_TRON_DEPOSIT,
  reset = false
) {
  try {
    if (reset) {
    ...
    for (let i = 0; i < transactions.length; i++) {
      ...
      if (transaction.receivedAmount > 1)
        await volumeTransactionModel.updateOne(
          { tx: t.transaction_id },
          { $set: transaction },
          { upsert: true }
        );
...
async function loadTransactionsExcel() {
  ...
  for (let i = 0; i < json.length; i++) {
    ...
    if (element["Date"] != "" && element["Received Amount (USD)"] != "") {
      ...
      let exist = await volumeTransactionModel.findOne({
        tx: tx,
      ...
      if (exist == null) {
        items.push({
          ...
          date: new Date(element["Date"]),
          receivedAmount: receivedAmount,
          receivedAmountEUR: parseCurrencyString(
            element["Received Amount (EUR)"]
          ),
...
async function getTransactions(
  wallet = "0x62c74109d073d5bd3cf6b4e6a91a77c3d4cf310a"
) {
  // await volumeTransactionModel.deleteMany({});
...
async function getTransactionDetails(txHash) {
  try {
    ...
    let providerr = new AlchemyProvider(
      Network.ETH_MAINNET,
      ALCHEMY_API_KEY,
      5
    );
    ...
    for (const log of receipt.logs) {
      if (
        ...
        return {
          ...
          symbol,
          hash: txHash,
          value: divideByDecimals(value + "", decimals),
        };
      }
    }
  } catch (error) {
...
//     if (matchedTransaction.to.toLowerCase() != "0xdfe0b33b515b36d640f26669cd4ee1af514680d5".toLowerCase()) {
...
router.get("/totalReceivedAmountByMonth", async (req, res) => {
  try {
    const results =
      await VolumeTransactionController.getTotalReceivedAmountByMonth();
    res.json(response(results, "success"));
  } catch (error) {
    handleHttpError(error, res);
  }
...
router.get("/:id", apiKeyMaster, async (req, res) => {
  try {
    const transaction = await VolumeTransactionController.getTransactionById(
      req.params.id
    );
    if (!transaction) {
...
router.put("/:id", apiKeyMaster, async (req, res) => {
  try {
    const transactionData = req.body;
    transactionData.receivedAmount = parseCurrencyString(
    ...
    const transaction = await VolumeTransactionController.updateTransaction(
      { _id: req.params.id },
      transactionData
    );
    if (!transaction) {
...
router.delete("/:id", apiKeyMaster, async (req, res) => {
  try {
    const transaction = await VolumeTransactionController.deleteTransaction({
      _id: req.params.id,
    });
    if (!transaction) {
      return res.status(404).json(response("Transaction not found", "error"));
